...
export async function mobileData(userId: string, role: string) {
  ...
  return { courses: result.rows, sessions: sessions.rows.map(s=>({...s,startDate:isoDate(s.startDate),endDate:isoDate(s.endDate)||isoDate(s.startDate),startTime:isoTime(s.startTime),endTime:isoTime(s.endTime)})).filter(s=>s.startDate&&s.startTime&&s.endTime) };
}
