...
export async function sendPush(token:string,environment:string,sessionID:string,days:number):Promise<number>{
 ...
 request.end(JSON.stringify({aps:{alert:{title:'Upcoming class',body:`Your class starts in ${days} day${days===1?'':'s'}. Open Tertiary Learning for details.`},sound:'default'},sessionID}));
 });
}
