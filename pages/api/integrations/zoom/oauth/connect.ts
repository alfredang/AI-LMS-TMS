...
import { ensureZoomColumns, getZoomCredentials, getZoomRedirectUri } from '../../../../../lib/zoom/client';

const ZOOM_AUTHORIZE_URL = 'https://zoom.us/oauth/authorize';
const ZOOM_SCOPES = process.env.ZOOM_SCOPES || ['user:read:user', 'meeting:write:meeting'].join(' ');
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const authUrl = `${ZOOM_AUTHORIZE_URL}?response_type=code&client_id=${encodeURIComponent(credentials.clientId)}&redirect_uri=${encodeURIComponent(redirectUri)}&scope=${encodeURIComponent(ZOOM_SCOPES)}&state=${encodeURIComponent(state)}`;
