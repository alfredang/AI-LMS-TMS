...
/**
 * GET /api/admin/auto-send-courseware-attendance-logs?limit=500
 *
...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    `);
    await pool.query(
      `CREATE INDEX IF NOT EXISTS idx_auto_send_courseware_attendance_log_run_id
       ON auto_send_courseware_attendance_log(run_id, created_at DESC)`
    );

    const result = await pool.query(
      `SELECT id, run_id, created_at, course_run_id, course_code, course_title,
              learner_name, learner_email, status, error_message
       FROM auto_send_courseware_attendance_log
    ...
    console.error('Error fetching auto send courseware attendance logs:', error);
