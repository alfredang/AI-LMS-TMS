  ...
  try {
    const result = await pool.query(
      `SELECT
         ...
         COALESCE((SELECT COALESCE(sg.approved_grant_amount, sg.estimated_grant_amount) FROM ssg_grants sg WHERE sg.enrollment_id = e.enrolment_id ORDER BY sg.created_date DESC LIMIT 1), e.grant_amount) AS grant_amount,
