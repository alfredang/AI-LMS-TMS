  ...
  try {
    const result = await pool.query(
      `SELECT
        ...
        c.course_code,
        (SELECT COUNT(*) FROM enrollment e WHERE e.course_run_id = cr.id AND LOWER(COALESCE(e.enrolment_status, '')) NOT IN ('admin removed', 'cancelled', 'withdrawn')) AS enrolled_learners
      FROM course_run cr
