...
/**
 * POST /api/external/cancel-session
 *
 * Delete (cancel) a single SSG session from a course run.
 * The session is removed from SSG, then the local DB mirror is refreshed,
 * then the GCal event is reconciled to drop the deleted session day.
 *
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // 5. Build SSG delete-sessions payload
    const payload = buildDeleteSessionsPayload({
      courseReferenceNumber,
      runData,
      sessions: [{
        id: sessRow.ssg_session_id,
        startDate: sessRow.start_date ? String(sessRow.start_date).slice(0, 10) : undefined,
        endDate: sessRow.end_date ? String(sessRow.end_date).slice(0, 10) : undefined,
        startTime: sessRow.start_time || undefined,
        endTime: sessRow.end_time || undefined,
    ...
    console.log(`[external/cancel-session] ${session_id} on ${ssgRunId}`);
    ...
    console.error('[external/cancel-session] error:', err);
