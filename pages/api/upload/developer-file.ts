...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Set upload directory based on file type - DEVELOPER-SPECIFIC PATH
    const uploadDir = path.join(process.cwd(), 'public', 'uploads', 'developer', fileType as string);
    ...
    console.log(`📁 Developer Upload: Uploading ${fileType} to ${uploadDir}`);
    ...
    // Delete old file if specified (for CV replacement)
    if (oldFileUrl && typeof oldFileUrl === 'string') {
      try {
        ...
        console.log(`🗑️ Developer Upload: Attempting to delete old file: ${oldFilePath}`);
        
        if (fs.existsSync(oldFilePath)) {
          fs.unlinkSync(oldFilePath);
          console.log(`✅ Developer Upload: Successfully deleted old file: ${oldFileName}`);
        } else {
          console.log(`⚠️ Developer Upload: Old file not found: ${oldFilePath}`);
        }
      } catch (deleteError) {
        console.error('❌ Developer Upload: Error deleting old file:', deleteError);
    ...
    const fileUrl = `${getBaseUrl().replace(/\/$/, '')}/uploads/developer/${fileType}/${fileName}`;
    
    console.log(`✅ Developer Upload: File uploaded successfully: ${fileName}`);
    console.log(`📁 Developer Upload: File URL: ${fileUrl}`);
