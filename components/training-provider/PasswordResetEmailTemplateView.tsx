...
const DEFAULT_SUBJECT = '{COMPANY_NAME} - Password Reset';

const DEFAULT_BODY = `<div style="font-family: Arial, sans-serif; font-size: 14px; color: #333; line-height: 1.6; max-width: 600px;">
  <p>Hi {USER_NAME},</p>
...
const PasswordResetEmailTemplateView: React.FC = () => {
  ...
  const fetchTemplate = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(getApiUrl('/api/training-provider/password-reset-email-template'));
      const data = await response.json();
      if (data.success && data.data) {
        const s = data.data.passwordResetEmailSubject || '';
        const b = data.data.passwordResetEmailBody || '';
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch(getApiUrl('/api/training-provider/password-reset-email-template'), {
        ...
        body: JSON.stringify({ passwordResetEmailSubject: subject, passwordResetEmailBody: body }),
      });
      const data = await response.json();
      if (data.success) {
        setOriginalSubject(subject);
        setOriginalBody(body);
        setSaveMessage({ type: 'success', text: 'Password reset email template saved successfully.' });
      } else {
        setSaveMessage({ type: 'error', text: data.error || 'Failed to save template.' });
      }
    } catch (error) {
      console.error('Error saving password reset email template:', error);
  ...
  const handleSendTestEmail = async () => {
    ...
    try {
      const res = await fetch(getApiUrl('/api/training-provider/send-test-email'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ testEmail: testEmail.trim(), subject, body, templateType: 'password-reset' }) });
  ...
  const variables = [
    ...
    { name: '{USER_EMAIL}', desc: "Recipient's email address" },
    { name: '{TEMP_PASSWORD}', desc: 'The generated temporary password' },
    { name: '{COMPANY_NAME}', desc: 'Your company name from Company Settings' },
    { name: '{COMPANY_SHORT_NAME}', desc: 'Your company short name' },
  ];
  ...
  const replaceVars = (text: string) =>
    text
      ...
      .replace(/\{TEMP_PASSWORD\}/g, 'a1b2c3d4')
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 max-w-4xl text-sm text-gray-500 dark:text-gray-400">
              Customise the email sent to users when they request a password reset via "Forgot Password".
            </p>
          </div>
...
export default PasswordResetEmailTemplateView;
