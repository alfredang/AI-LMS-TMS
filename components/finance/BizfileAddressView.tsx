...
export default function BizfileAddressView() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const resp = await fetch(`/api/bizfile/query?endpoint=entityRegisteredAddress&uen=${encodeURIComponent(uen.trim())}`);
  ...
  const addr = entity?.registeredAddress || entity;

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Entity Registered Address</h1>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Retrieve registered address details. <span className="text-xs opacity-70">(1 API call)</span></p>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              ...
              { label: 'Block', value: addr.block || addr.blkhseNo },
              { label: 'Street', value: addr.streetName },
              { label: 'Level', value: addr.level || addr.levelNo },
              { label: 'Unit', value: addr.unit || addr.unitNo },
              { label: 'Building', value: addr.buildingName || addr.bldgName },
              { label: 'Postal Code', value: addr.postalCode },
              { label: 'Change Date', value: addr.changeDate },
