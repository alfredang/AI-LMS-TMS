...
const DeveloperSidebar: React.FC<DeveloperSidebarProps> = ({ onNavigate, collapsed = false }) => {
    ...
    return (
        <nav className="space-y-6 p-4 bg-white dark:bg-slate-800 text-gray-900 dark:text-white h-full">
            ...
            {collapsed ? (
                <a
                    href="https://jobsandskills.skillsfuture.gov.sg/frameworks/skills-frameworks#download-the-latest-skills-framework-dataset"
                    target="_blank"
                    rel="noopener noreferrer"
                    title="Skills Frameworks — SFw Dataset"
                    className={`group flex items-center justify-center px-0 rounded-xl py-2.5 text-sm font-medium transition-all duration-150 ${inactiveClass}`}
                >
                    <Icon
                ...
                <NavSection title="Skills Frameworks" icon={IconName.Library} isOpen={openSections.skillsFrameworks} onToggle={() => toggleSection('skillsFrameworks')} collapsed={collapsed}>
                    <a
                        href="https://jobsandskills.skillsfuture.gov.sg/frameworks/skills-frameworks#download-the-latest-skills-framework-dataset"
                        target="_blank"
                        rel="noopener noreferrer"
                        className={`group flex items-center gap-3 px-3 pl-8 rounded-xl py-2.5 text-sm font-medium transition-all duration-150 ${inactiveClass}`}
                    >
                        <span className="truncate">SFw Dataset</span>
                        <svg className="w-3.5 h-3.5 text-gray-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                        </svg>
                    </a>
                    <a
                        href="https://www.tpgateway.gov.sg/plan-courses/course-accreditation"
                        target="_blank"
                        rel="noopener noreferrer"
                        className={`group flex items-center gap-3 px-3 pl-8 rounded-xl py-2.5 text-sm font-medium transition-all duration-150 ${inactiveClass}`}
                    >
                        <span className="truncate">Latest CP</span>
                        <svg className="w-3.5 h-3.5 text-gray-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                        </svg>
                    </a>
