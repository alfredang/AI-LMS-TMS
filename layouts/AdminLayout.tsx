...
const PAGE_LABELS: Partial<Record<AdminPage, string>> = {
  ...
  [AdminPage.GrantCalculator]: 'Grant Calculator',
...
const AdminLayout: React.FC = () => {
  const { currentView, adminPage, selectedCourse, editingCourse, selectedCourseRunId, editingCourseRun, setAdminPage } = useLms();
  ...
  const renderContent = () => {
    ...
    switch (adminPage) {
      ...
      case AdminPage.GrantCalculator:
        ...
        <div className="flex flex-shrink-0">
          <aside className={`${isSidebarOpen ? 'w-64' : 'w-14'} bg-white dark:bg-slate-800 border-r border-gray-200 dark:border-slate-700 flex flex-col h-[calc(100vh-64px)] sticky top-[64px] transition-all duration-200`}>
            ...
            <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
              <AdminSidebar onSelectWorkflow={setSelectedWorkflowId} collapsed={!isSidebarOpen} />
            </div>
